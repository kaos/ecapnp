%%% DO NOT EDIT, this file was generated.
-include_lib("ecapnp/include/ecapnp.hrl").

-compile({nowarn_unused_function, schema/1}).
-compile({nowarn_unused_function, schema/2}).
-compile({nowarn_unused_function, schema/3}).
-compile({nowarn_unused_function, schema/4}).

%% Write value to object field.
%% -spec schema(set, Field::atom(), Value::term(), Object::#object{}) -> ok.
schema(set, Field, Value, Object) ->
    ecapnp:set(Field, Value, Object).

%% Get a reference to the root object in message.
%% -spec schema(root, Type::atom() | integer(), Message::list(binary())) -> {ok, Root::#object{}}.
schema(root, Type, Message) ->
    ecapnp:get_root(Type, schema(schema), Message);

%% Read object field value.
%% -spec schema(get, Field::atom(), Object::#object{}) -> term().
schema(get, Field, Object) ->
    ecapnp:get(Field, Object);

%% Type cast object to another struct or list.
%% -spec schema(to_struct, Type::atom() | integer(), Object1::#object{}) -> Object2#object{}.
%% -spec schema(to_list, Type::atom() | integer(), Object::#object{}) -> list().
schema(TypeCast, Type, Object)
  when TypeCast == to_struct;
       TypeCast == to_list ->
    ecapnp_obj:TypeCast(Type, Object).

%% Set root type for a new message.
%% -spec schema(root, Type::atom() | integer()) -> {ok, Root::#object{}}.
schema(root, Type) ->
    ecapnp:set_root(Type, schema(schema));

%% Read unnamed union value of object.
%% -spec schema(get, Object::#object{}) -> Tag::atom() | {Tag::atom(), Value::term()}.
schema(get, Object) ->
    ecapnp:get(Object);

%% Type cast object to text/data.
%% -spec schema(to_text | to_data, Object::#object{}) -> binary().
schema(TypeCast, Object)
  when TypeCast == to_text;
       TypeCast == to_data ->
    ecapnp_obj:TypeCast(Object).

%% Get the compiled schema.
%% -spec schema(schema) -> #schema{}.

schema(schema) ->
  #schema{
    node=#node{ %% 0xa93fc509624c72d9
      name=schema, id=12195682960037147353, source= <<"schema.capnp">> },
    types=
      [#struct{
         node=#node{ %% 0xe682ab4cf923a417
           name='Node', id=16610026722781537303, source= <<"schema.capnp:Node">> },
         dsize=5, psize=5, esize=inlineComposite,
         union_field=#data{ align=96, default= 0, type=
           {union,
             [{0,file,void},
              {1,struct,{group,11430331134483579957}},
              {2,enum,{group,13063450714778629528}},
              {3,interface,{group,16728431493453586831}},
              {4,const,{group,12793219851699983392}},
              {5,annotation,{group,17011813041836786320}}
           ]} },
         fields=
           [{id,
             #data{ type=uint64, align=0,
                    default= 0 }},
            {displayName,
             #ptr{ type=text, idx=0,
                   default= <<>> }},
            {displayNamePrefixLength,
             #data{ type=uint32, align=64,
                    default= 0 }},
            {scopeId,
             #data{ type=uint64, align=128,
                    default= 0 }},
            {nestedNodes,
             #ptr{ type={list,{struct,16050641862814319170}}, idx=1,
                   default= [] }},
            {annotations,
             #ptr{ type={list,{struct,17422339044421236034}}, idx=2,
                   default= [] }}
           ],
         types=
           [#struct{
              node=#node{ %% 0xdebf55bbfa0fc242
                name='NestedNode', id=16050641862814319170, source= <<"schema.capnp:Node.NestedNode">> },
              dsize=1, psize=1, esize=inlineComposite,
              union_field=none,
              fields=
                [{name,
                  #ptr{ type=text, idx=0,
                        default= <<>> }},
                 {id,
                  #data{ type=uint64, align=0,
                         default= 0 }}
                ]},
            #struct{
              node=#node{ %% 0xec1619d4400a0290
                name=annotation, id=17011813041836786320, source= <<"schema.capnp:Node.annotation">> },
              dsize=5, psize=5, esize=inlineComposite,
              union_field=none,
              fields=
                [{type,
                  #ptr{ type={struct,15020482145304562784}, idx=3,
                        default= <<0,0,0,0,0,0,0,0>> }},
                 {targetsFile,
                  #data{ type=bool, align=119,
                         default= false }},
                 {targetsConst,
                  #data{ type=bool, align=118,
                         default= false }},
                 {targetsEnum,
                  #data{ type=bool, align=117,
                         default= false }},
                 {targetsEnumerant,
                  #data{ type=bool, align=116,
                         default= false }},
                 {targetsStruct,
                  #data{ type=bool, align=115,
                         default= false }},
                 {targetsField,
                  #data{ type=bool, align=114,
                         default= false }},
                 {targetsUnion,
                  #data{ type=bool, align=113,
                         default= false }},
                 {targetsGroup,
                  #data{ type=bool, align=112,
                         default= false }},
                 {targetsInterface,
                  #data{ type=bool, align=127,
                         default= false }},
                 {targetsMethod,
                  #data{ type=bool, align=126,
                         default= false }},
                 {targetsParam,
                  #data{ type=bool, align=125,
                         default= false }},
                 {targetsAnnotation,
                  #data{ type=bool, align=124,
                         default= false }}
                ]},
            #struct{
              node=#node{ %% 0xb18aa5ac7a0d9420
                name=const, id=12793219851699983392, source= <<"schema.capnp:Node.const">> },
              dsize=5, psize=5, esize=inlineComposite,
              union_field=none,
              fields=
                [{type,
                  #ptr{ type={struct,15020482145304562784}, idx=3,
                        default= <<0,0,0,0,0,0,0,0>> }},
                 {value,
                  #ptr{ type={struct,14853958794117909659}, idx=4,
                        default= <<0,0,0,0,0,0,0,0>> }}
                ]},
            #struct{
              node=#node{ %% 0xe82753cff0c2218f
                name=interface, id=16728431493453586831, source= <<"schema.capnp:Node.interface">> },
              dsize=5, psize=5, esize=inlineComposite,
              union_field=none,
              fields=
                [{methods,
                  #ptr{ type={list,{struct,10736806783679155584}}, idx=3,
                        default= [] }}
                ]},
            #struct{
              node=#node{ %% 0xb54ab3364333f598
                name=enum, id=13063450714778629528, source= <<"schema.capnp:Node.enum">> },
              dsize=5, psize=5, esize=inlineComposite,
              union_field=none,
              fields=
                [{enumerants,
                  #ptr{ type={list,{struct,10919677598968879693}}, idx=3,
                        default= [] }}
                ]},
            #struct{
              node=#node{ %% 0x9ea0b19b37fb4435
                name=struct, id=11430331134483579957, source= <<"schema.capnp:Node.struct">> },
              dsize=5, psize=5, esize=inlineComposite,
              union_field=none,
              fields=
                [{dataWordCount,
                  #data{ type=uint16, align=112,
                         default= 0 }},
                 {pointerCount,
                  #data{ type=uint16, align=192,
                         default= 0 }},
                 {preferredListEncoding,
                  #data{ type={enum,15102134695616452902}, align=208,
                         default= 0 }},
                 {isGroup,
                  #data{ type=bool, align=231,
                         default= false }},
                 {discriminantCount,
                  #data{ type=uint16, align=240,
                         default= 0 }},
                 {discriminantOffset,
                  #data{ type=uint32, align=256,
                         default= 0 }},
                 {fields,
                  #ptr{ type={list,{struct,11145653318641710175}}, idx=3,
                        default= [] }}
                ]}
           ]},
       #struct{
         node=#node{ %% 0x9aad50a41f4af45f
           name='Field', id=11145653318641710175, source= <<"schema.capnp:Field">> },
         dsize=3, psize=4, esize=inlineComposite,
         union_field=#data{ align=64, default= 0, type=
           {union,
             [{0,slot,{group,14133145859926553711}},
              {1,group,{group,14626792032033250577}}
           ]} },
         fields=
           [{name,
             #ptr{ type=text, idx=0,
                   default= <<>> }},
            {codeOrder,
             #data{ type=uint16, align=0,
                    default= 0 }},
            {annotations,
             #ptr{ type={list,{struct,17422339044421236034}}, idx=1,
                   default= [] }},
            {discriminantValue,
             #data{ type=uint16, align=16,
                    default= 65535 }},
            {ordinal,
             #group{ id=13515537513213004774 }}
           ],
         types=
           [#struct{
              node=#node{ %% 0xbb90d5c287870be6
                name=ordinal, id=13515537513213004774, source= <<"schema.capnp:Field.ordinal">> },
              dsize=3, psize=4, esize=inlineComposite,
              union_field=#data{ align=80, default= 0, type=
                {union,
                  [{0,implicit,void},
                   {1,explicit,{data,uint16,96,0}}
                ]} }},
            #struct{
              node=#node{ %% 0xcafccddb68db1d11
                name=group, id=14626792032033250577, source= <<"schema.capnp:Field.group">> },
              dsize=3, psize=4, esize=inlineComposite,
              union_field=none,
              fields=
                [{typeId,
                  #data{ type=uint64, align=128,
                         default= 0 }}
                ]},
            #struct{
              node=#node{ %% 0xc42305476bb4746f
                name=slot, id=14133145859926553711, source= <<"schema.capnp:Field.slot">> },
              dsize=3, psize=4, esize=inlineComposite,
              union_field=none,
              fields=
                [{offset,
                  #data{ type=uint32, align=32,
                         default= 0 }},
                 {type,
                  #ptr{ type={struct,15020482145304562784}, idx=2,
                        default= <<0,0,0,0,0,0,0,0>> }},
                 {defaultValue,
                  #ptr{ type={struct,14853958794117909659}, idx=3,
                        default= <<0,0,0,0,0,0,0,0>> }}
                ]}
           ]},
       #struct{
         node=#node{ %% 0x978a7cebdc549a4d
           name='Enumerant', id=10919677598968879693, source= <<"schema.capnp:Enumerant">> },
         dsize=1, psize=2, esize=inlineComposite,
         union_field=none,
         fields=
           [{name,
             #ptr{ type=text, idx=0,
                   default= <<>> }},
            {codeOrder,
             #data{ type=uint16, align=0,
                    default= 0 }},
            {annotations,
             #ptr{ type={list,{struct,17422339044421236034}}, idx=1,
                   default= [] }}
           ]},
       #struct{
         node=#node{ %% 0x9500cce23b334d80
           name='Method', id=10736806783679155584, source= <<"schema.capnp:Method">> },
         dsize=1, psize=4, esize=inlineComposite,
         union_field=none,
         fields=
           [{name,
             #ptr{ type=text, idx=0,
                   default= <<>> }},
            {codeOrder,
             #data{ type=uint16, align=0,
                    default= 0 }},
            {params,
             #ptr{ type={list,{struct,14681955158633610486}}, idx=1,
                   default= [] }},
            {requiredParamCount,
             #data{ type=uint16, align=16,
                    default= 0 }},
            {returnType,
             #ptr{ type={struct,15020482145304562784}, idx=2,
                   default= <<0,0,0,0,0,0,0,0>> }},
            {annotations,
             #ptr{ type={list,{struct,17422339044421236034}}, idx=3,
                   default= [] }}
           ],
         types=
           [#struct{
              node=#node{ %% 0xcbc0c86dae91fcf6
                name='Param', id=14681955158633610486, source= <<"schema.capnp:Method.Param">> },
              dsize=0, psize=4, esize=inlineComposite,
              union_field=none,
              fields=
                [{name,
                  #ptr{ type=text, idx=0,
                        default= <<>> }},
                 {type,
                  #ptr{ type={struct,15020482145304562784}, idx=1,
                        default= <<0,0,0,0,0,0,0,0>> }},
                 {defaultValue,
                  #ptr{ type={struct,14853958794117909659}, idx=2,
                        default= <<0,0,0,0,0,0,0,0>> }},
                 {annotations,
                  #ptr{ type={list,{struct,17422339044421236034}}, idx=3,
                        default= [] }}
                ]}
           ]},
       #struct{
         node=#node{ %% 0xd07378ede1f9cc60
           name='Type', id=15020482145304562784, source= <<"schema.capnp:Type">> },
         dsize=2, psize=1, esize=inlineComposite,
         union_field=#data{ align=0, default= 0, type=
           {union,
             [{0,void,void},
              {1,bool,void},
              {2,int8,void},
              {3,int16,void},
              {4,int32,void},
              {5,int64,void},
              {6,uint8,void},
              {7,uint16,void},
              {8,uint32,void},
              {9,uint64,void},
              {10,float32,void},
              {11,float64,void},
              {12,text,void},
              {13,data,void},
              {14,list,{group,9792858745991129751}},
              {15,enum,{group,11389172934837766057}},
              {16,struct,{group,12410354185295152851}},
              {17,interface,{group,17116997365232503999}},
              {18,object,void}
           ]} },
         types=
           [#struct{
              node=#node{ %% 0xed8bca69f7fb0cbf
                name=interface, id=17116997365232503999, source= <<"schema.capnp:Type.interface">> },
              dsize=2, psize=1, esize=inlineComposite,
              union_field=none,
              fields=
                [{typeId,
                  #data{ type=uint64, align=64,
                         default= 0 }}
                ]},
            #struct{
              node=#node{ %% 0xac3a6f60ef4cc6d3
                name=struct, id=12410354185295152851, source= <<"schema.capnp:Type.struct">> },
              dsize=2, psize=1, esize=inlineComposite,
              union_field=none,
              fields=
                [{typeId,
                  #data{ type=uint64, align=64,
                         default= 0 }}
                ]},
            #struct{
              node=#node{ %% 0x9e0e78711a7f87a9
                name=enum, id=11389172934837766057, source= <<"schema.capnp:Type.enum">> },
              dsize=2, psize=1, esize=inlineComposite,
              union_field=none,
              fields=
                [{typeId,
                  #data{ type=uint64, align=64,
                         default= 0 }}
                ]},
            #struct{
              node=#node{ %% 0x87e739250a60ea97
                name=list, id=9792858745991129751, source= <<"schema.capnp:Type.list">> },
              dsize=2, psize=1, esize=inlineComposite,
              union_field=none,
              fields=
                [{elementType,
                  #ptr{ type={struct,15020482145304562784}, idx=0,
                        default= <<0,0,0,0,0,0,0,0>> }}
                ]}
           ]},
       #struct{
         node=#node{ %% 0xce23dcd2d7b00c9b
           name='Value', id=14853958794117909659, source= <<"schema.capnp:Value">> },
         dsize=2, psize=1, esize=inlineComposite,
         union_field=#data{ align=0, default= 0, type=
           {union,
             [{0,void,void},
              {1,bool,{data,bool,23,false}},
              {2,int8,{data,int8,16,0}},
              {3,int16,{data,int16,16,0}},
              {4,int32,{data,int32,32,0}},
              {5,int64,{data,int64,64,0}},
              {6,uint8,{data,uint8,16,0}},
              {7,uint16,{data,uint16,16,0}},
              {8,uint32,{data,uint32,32,0}},
              {9,uint64,{data,uint64,64,0}},
              {10,float32,{data,float32,32,0.0}},
              {11,float64,{data,float64,64,0.0}},
              {12,text,{ptr,text,0,<<>>}},
              {13,data,{ptr,data,0,<<>>}},
              {14,list,{ptr,object,0,<<0,0,0,0,0,0,0,0>>}},
              {15,enum,{data,uint16,16,0}},
              {16,struct,{ptr,object,0,<<0,0,0,0,0,0,0,0>>}},
              {17,interface,void},
              {18,object,{ptr,object,0,<<0,0,0,0,0,0,0,0>>}}
           ]} }},
       #struct{
         node=#node{ %% 0xf1c8950dab257542
           name='Annotation', id=17422339044421236034, source= <<"schema.capnp:Annotation">> },
         dsize=1, psize=1, esize=inlineComposite,
         union_field=none,
         fields=
           [{id,
             #data{ type=uint64, align=0,
                    default= 0 }},
            {value,
             #ptr{ type={struct,14853958794117909659}, idx=0,
                   default= <<0,0,0,0,0,0,0,0>> }}
           ]},
       #enum{
         node=#node{ %% 0xd1958f7dba521926
           name='ElementSize', id=15102134695616452902, source= <<"schema.capnp:ElementSize">> },
         values=
           [{0,empty},
            {1,bit},
            {2,byte},
            {3,twoBytes},
            {4,fourBytes},
            {5,eightBytes},
            {6,pointer},
            {7,inlineComposite}
           ]},
       #struct{
         node=#node{ %% 0xbfc546f6210ad7ce
           name='CodeGeneratorRequest', id=13818529054586492878, source= <<"schema.capnp:CodeGeneratorRequest">> },
         dsize=0, psize=2, esize=inlineComposite,
         union_field=none,
         fields=
           [{nodes,
             #ptr{ type={list,{struct,16610026722781537303}}, idx=0,
                   default= [] }},
            {requestedFiles,
             #ptr{ type={list,{struct,14981803260258615394}}, idx=1,
                   default= [] }}
           ],
         types=
           [#struct{
              node=#node{ %% 0xcfea0eb02e810062
                name='RequestedFile', id=14981803260258615394, source= <<"schema.capnp:CodeGeneratorRequest.RequestedFile">> },
              dsize=1, psize=2, esize=inlineComposite,
              union_field=none,
              fields=
                [{id,
                  #data{ type=uint64, align=0,
                         default= 0 }},
                 {filename,
                  #ptr{ type=text, idx=0,
                        default= <<>> }},
                 {imports,
                  #ptr{ type={list,{struct,12560611460656617445}}, idx=1,
                        default= [] }}
                ],
              types=
                [#struct{
                   node=#node{ %% 0xae504193122357e5
                     name='Import', id=12560611460656617445, source= <<"schema.capnp:CodeGeneratorRequest.RequestedFile.Import">> },
                   dsize=1, psize=1, esize=inlineComposite,
                   union_field=none,
                   fields=
                     [{id,
                       #data{ type=uint64, align=0,
                              default= 0 }},
                      {name,
                       #ptr{ type=text, idx=0,
                             default= <<>> }}
                     ]}
                ]}
           ]}
      ]}.
